...
# datasets
datasets <- "CMIP6Amon"# "CMIP6"
a <- "day"
if (datasets == "CMIP6Amon") a <- "mon"
...
ls <- grep(paste0("AtlasHub-inventory/Hub/", gsub("Amon", "", datasets), "_", a), filelist, value = TRUE, fixed = TRUE) %>% grep("\\.csv$", ., value = TRUE)
...
# csv
filename <- paste0(out.dir, "/", gsub(".csv", paste0("_", gsub("-", "", Sys.Date()), ".csv"), gsub(".*/", "", ls)))
write.csv(diff, filename)

# figure
diff.p <- as.matrix(diff[,-(1:2)])
