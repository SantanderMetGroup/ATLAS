...
ls <- grep(paste0("ESGF-inventory/CMIP6/", datasets, extension,".csv"), filelist, value = TRUE, fixed = TRUE) %>% grep("\\.csv$", ., value = TRUE)
esgffile <- paste0(root, ls)
ls <- grep(paste0("AtlasHub-inventory/Hub/", datasets, "_Hub.csv"), filelist, value = TRUE, fixed = TRUE) %>% grep("\\.csv$", ., value = TRUE)
...
# csv
filename <- paste0(out.dir, "/", gsub(".csv", paste0("_vs_ESGF", extension, "_", gsub("-", "", Sys.Date()), ".csv"), gsub(".*/", "", ls)))
