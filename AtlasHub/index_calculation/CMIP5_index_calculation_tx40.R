...
years <- years[which(years>1949)]
    lapply(years, function(x){
      if (!file.exists(paste0(out.dir, "tx40/", cmip[i], "_tx40_monthly_", x, ".nc4"))) {
        
        print(paste0(x, "----------------------------------------------------------------"))
        index <- climate4R.chunk(n.chunks = 2,
                                 C4R.FUN.args = list(FUN = "indexGrid",
                                                     tx = list(dataset = cmip[i], var = "tasmax"),
                                                     index.code = "TXth",
        ...
        message(x, ".........", i)
        grid2nc(index, NetCDFOutFile = paste0(out.dir, "tx40/", cmip[i], "_tx40_monthly_", x, ".nc4"))
      }
    })
  } 
})
