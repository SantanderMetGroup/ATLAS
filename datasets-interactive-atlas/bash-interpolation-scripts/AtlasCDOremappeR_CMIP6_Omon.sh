...
# Title: Interpolation of montlhy CMIP6 ocean monthly data
# Description:
#   The script AtlasCDOremappeR_CMIP6_Omon.sh was used to perform the
#   interpolation of all montlhy CMIP6 ocean monthly data (variables siconc, ph,
#   and tos) data to the common grid at 1 degree resolution.  To run the script:
#  	 
...
#
#   This procedure was applied to interpolate the CMIP6 outputs from the ocean
#   models that have irregular grids which crashed due to lack of informations
#   when using the basic conservative interpolation described above.  The data
#   for the crashed models were interpolated on the finer grid (0.5 degree -
#   360x720) using distance-weighted average remapping (remapdis) in order to get
#   the data on the regular grid. The finer grid was chosen in order to lose as
...
#   ****************************************************************************************
#   NOTE:
#   The script also performs final masking of all the interpolated data to a
#   common land-sea mask if wanted.  In that case, additional information on the
#   full path to the common land-sea mask needs to be provided by a user.  For
#   ATLAS land_sea_mask_1degree.nc4 was use for the final masking.
#   ****************************************************************************************
# Author: J. Milovac
