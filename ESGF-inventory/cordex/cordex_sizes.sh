...
query_esgf "project=CORDEX&product=output&experiment=${experiments}&time_frequency=${frequencies}&domain=EUR-11,EUR-22,EUR-44" 2>/dev/null | jq --slurp 'map(.size)|add' | numfmt --to=iec
