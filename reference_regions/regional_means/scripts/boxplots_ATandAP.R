...
prepareData <- function(allfiles, modelruns, aggr.fun, periods, exp){
      ...
      dat <- lapply(1:length(modelruns), function(i) {
            ...
            l1 <- lapply(1:length(exp), FUN = function(j) {
                  ...
                  dates <- tryCatch({
                        yrs <- grep(exp[j], modelfiles, value = TRUE) %>% read.table(header = TRUE, sep = ",", skip = 7) %>% subset(select = "date", drop = TRUE) %>% gsub("-.*", "", .) %>% as.integer()
                        rcp <- lapply(split(rcp[,-1], f = yrs), function(x) apply(x, MARGIN = 2, FUN = aggr.fun, na.rm = TRUE))
                  }, error = function(err) return(NULL))
                  if (!is.null(rcp)) {
                        rcp <- do.call("rbind", rcp)
                        delta <- lapply(periods, function(k){
...
prWL.cmip5 <- prepareData(allfiles, modelruns, aggr.fun, periods = list("+1.5º" = per1.5, "+2º" = per2), exp)

prWLmediana.cmip5 <- lapply(prWL.cmip5, apply, 2, median, na.rm = T)
prWLp90.cmip5 <- lapply(prWL.cmip5, apply, 2, quantile, 0.9, na.rm = T)
prWLp10.cmip5 <- lapply(prWL.cmip5, apply, 2, quantile, 0.1, na.rm = T)
...
prWL.cmip6 <- prepareData(allfiles, modelruns, aggr.fun, periods = list("+1.5º" = per1.5, "+2º" = per2), exp)

prWLmediana.cmip6 <- lapply(prWL.cmip6, apply, 2, median, na.rm = T)
prWLp90.cmip6 <- lapply(prWL.cmip6, apply, 2, quantile, 0.9, na.rm = T)
...
pr.cmip5 <- prepareData(allfiles, modelruns, aggr.fun, periods, exp)
...
pr.cmip6 <- prepareData(allfiles, modelruns, aggr.fun, periods, exp)
...
p <- lapply(1:length(prmediana.cmip5), function(i){
      col = c("darkmagenta", "darkgoldenrod1", "green", "blue", "red")
      a1 <- c(prWLmediana.cmip5[[i]][1],prWLmediana.cmip6[[i]][1], rep(NA, 8))
      a2 <- c(rep(NA, 2),prWLmediana.cmip5[[i]][2],prWLmediana.cmip6[[i]][2], rep(NA, 6))
      a <- c(c(NA,NA,NA,NA),prmediana.cmip5[[i]][1], prmediana.cmip6[[i]][1], prmediana.cmip5[[i]][4], prmediana.cmip6[[i]][4],prmediana.cmip5[[i]][7], prmediana.cmip6[[i]][7])
      b <- c(c(NA,NA,NA,NA),prmediana.cmip5[[i]][2], prmediana.cmip6[[i]][2], prmediana.cmip5[[i]][5], prmediana.cmip6[[i]][5],prmediana.cmip5[[i]][8], prmediana.cmip6[[i]][8])
      d <- c(c(NA,NA,NA,NA),prmediana.cmip5[[i]][3], prmediana.cmip6[[i]][3], prmediana.cmip5[[i]][6], prmediana.cmip6[[i]][6],prmediana.cmip5[[i]][9], prmediana.cmip6[[i]][9])
      x <- c("+1.5º5","+1.5º6", "+2º5","+2º6", "anear5", "anear6", "bmid5", "bmid6", "clong5", "clong6")
      df <- data.frame("term" = x, "WL1" = a1, "WL2" = a2, "ssp126" = a, "ssp245" = b, "ssp585" = d)
      
      a1i <- c(prWLp10.cmip5[[i]][1],prWLp10.cmip6[[i]][1], rep(NA, 8))
      a2i <- c(rep(NA, 2), prWLp10.cmip5[[i]][2],prWLp10.cmip6[[i]][2], rep(NA, 6))
      ai <- c(c(NA,NA,NA,NA),prp10.cmip5[[i]][1], prp10.cmip6[[i]][1], prp10.cmip5[[i]][4], prp10.cmip6[[i]][4],prp10.cmip5[[i]][7], prp10.cmip6[[i]][7])
      bi <- c(c(NA,NA,NA,NA),prp10.cmip5[[i]][2], prp10.cmip6[[i]][2], prp10.cmip5[[i]][5], prp10.cmip6[[i]][5],prp10.cmip5[[i]][8], prp10.cmip6[[i]][8])
      di <- c(c(NA,NA,NA,NA),prp10.cmip5[[i]][3], prp10.cmip6[[i]][3], prp10.cmip5[[i]][6], prp10.cmip6[[i]][6],prp10.cmip5[[i]][9], prp10.cmip6[[i]][9])
      xi <- c("+1.5º5","+1.5º6", "+2º5","+2º6", "anear5", "anear6", "bmid5", "bmid6", "clong5", "clong6")
      dfi <- data.frame("term" = xi, "WL1" = a1i, "WL2" = a2i, "ssp126" = ai, "ssp245" = bi, "ssp585" = di)
      
      a1j <- c(prWLp90.cmip5[[i]][1],prWLp90.cmip6[[i]][1], rep(NA, 8))
      a2j <- c(rep(NA, 2),prWLp90.cmip5[[i]][2],prWLp90.cmip6[[i]][2], rep(NA, 6))
      aj <- c(c(NA,NA,NA,NA),prp90.cmip5[[i]][1], prp90.cmip6[[i]][1], prp90.cmip5[[i]][4], prp90.cmip6[[i]][4],prp90.cmip5[[i]][7], prp90.cmip6[[i]][7])
      bj <- c(c(NA,NA,NA,NA),prp90.cmip5[[i]][2], prp90.cmip6[[i]][2], prp90.cmip5[[i]][5], prp90.cmip6[[i]][5],prp90.cmip5[[i]][8], prp90.cmip6[[i]][8])
      dj <- c(c(NA,NA,NA,NA),prp90.cmip5[[i]][3], prp90.cmip6[[i]][3], prp90.cmip5[[i]][6], prp90.cmip6[[i]][6],prp90.cmip5[[i]][9], prp90.cmip6[[i]][9])
      xj <- c("+1.5º5","+1.5º6", "+2º5","+2º6", "anear5", "anear6", "bmid5", "bmid6", "clong5", "clong6")
      dfj <- data.frame("term" = xj, "WL1" = a1j, "WL2" = a2j, "ssp126" = aj, "ssp245" = bj, "ssp585" = dj)
