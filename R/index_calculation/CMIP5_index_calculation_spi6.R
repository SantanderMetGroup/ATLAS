...
years <- years[which(years>1949)]
    index <- climate4R.chunk(n.chunks = 2,
                    C4R.FUN.args = list(FUN = "speiGrid", pr.grid = list(dataset = cmip[i], var = "pr"), scale = 6),
...
index$Data[which(is.infinite(index$Data))] <- NA
	...
	index.x <- subsetGrid(index, years = x)
      grid2nc(index.x, NetCDFOutFile = paste0(out.dir, "spi6/", cmip[i], "_spi6_monthly_", x, ".nc4"))
    })
  } 
})
