...
years <- years[which(years>1949)]
    lapply(years, function(x){
      if (!file.exists(paste0(out.dir, "tasmin/", cmip[i], "_tasmin_monthly_", x, ".nc4"))) {
      print(paste0(x, "----------------------------------------------------------------"))
      index <- climate4R.chunk(n.chunks = 2,
                               C4R.FUN.args = list(FUN = "aggregateGrid",
                                                   grid = list(dataset = cmip[i], var = "tasmin"),
                                                   aggr.m = list(FUN = min, na.rm = TRUE)),
                               loadGridData.args = list(years = x))
      index <- redim(index, drop = TRUE)
      message(x, ".........", i)
      grid2nc(index, NetCDFOutFile = paste0(out.dir, "tasmin/", cmip[i], "_tasmin_monthly_", x, ".nc4"))
      }
    })
  } 
})
