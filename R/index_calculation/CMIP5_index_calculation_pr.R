...
years <- years[which(years>1949)]
    ...
    lapply(years, function(x){
      if (!file.exists(paste0(out.dir, "pr/", cmip[i], "_pr_monthly_", x, ".nc4"))) {
      print(paste0(x, "----------------------------------------------------------------"))
      index <- climate4R.chunk(n.chunks = 2,
                               C4R.FUN.args = list(FUN = "aggregateGrid",
                                                   grid = list(dataset = cmip[i], var = "pr"),
                                                   aggr.m = list(FUN = sum, na.rm = TRUE)),
                               loadGridData.args = list(years = x))
      index <- redim(index, drop = TRUE)
      message(x, ".........", i)
      grid2nc(index, NetCDFOutFile = paste0(out.dir, "pr/", cmip[i], "_pr_monthly_", x, ".nc4"))
      }
    })
  } 
})
