...
computeDeltas <- function(project, 
  ...
  myurl <- "https://api.github.com/repos/SantanderMetGroup/ATLAS/git/trees/mai-devel?recursive=1"
  
  ## for remote
  req <- GET(myurl) %>% stop_for_status(req)
  filelist <- unlist(lapply(content(req)$tree, "[", "path"), use.names = FALSE)
  root <- "https://raw.githubusercontent.com/SantanderMetGroup/ATLAS/mai-devel/"
