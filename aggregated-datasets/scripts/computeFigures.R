#     computeFigures.R Compute temperature and precipitation change figures from data files 
    ...
    sp <- lapply(1:length(scatter.seasons), function(k) {
      ...
      xyplot(y~x, data = dfs, xlim = xlimk, ylim = ylimk, pch = 19, aspect = "45", 
             ...
             panel = function(...){
               ...
               panel.xyplot(cmip5.off.s[[k]], cmip5.off.s[[k]], col = rgb(126/255, 128/255, 93/255, 0.5), pch = 19, cex = 1)
               panel.xyplot(cmip6.off.s[[k]], cmip6.off.s[[k]], col = rgb(126/255, 128/255, 93/255), cex = 1, pch = 19)
               panel.xyplot(...)
             })
    })
    c(list(bp), sp)
