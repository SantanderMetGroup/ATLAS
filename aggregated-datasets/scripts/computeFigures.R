computeFigures <- function(regions,
  ...
  p <- lapply(regions, function(region) {
    ...
    WL.cmip5 <- suppressMessages(computeDeltas(project, var = "tas", experiment, season = 1:12, ref.period, periods = WL, area, region)[[1]])
    ...
    WL.cmip6 <- suppressMessages(computeDeltas(project, var = "tas", experiment, season = 1:12, ref.period, periods = WL, area, region)[[1]])
    ...
    WL.cordex <- suppressMessages(computeDeltas(project, var = "tas", experiment, season = 1:12, ref.period, periods = WL, area, region))
    ...
    cmip5 <- suppressMessages(computeDeltas(project, var = "tas", experiment, season = 1:12, ref.period, periods, area, region)[[1]])
    ...
    cmip6 <- suppressMessages(computeDeltas(project, var = "tas", experiment, season = 1:12, ref.period, periods, area, region)[[1]])
    ...
    cordex <- suppressMessages(computeDeltas(project, var = "tas", experiment, season = 1:12, ref.period, periods, area, region))
    ...
    x0 <- c(paste0("near-", sc), paste0("mid-", sc), paste0("far-", sc),
    ...
    df <- data.frame("term" = x, "value" = unname(do.call("c", list(a, b, d, a1, a2, a3, a4)))[ind])
    ...
    dfi <- data.frame("term" = x, "value" = unname(do.call("c", list(ai, bi, di, a1i, a2i, a3i, a4i)))[ind])
    
    dfi.sub <- dfi 
    dfi.sub$value <- dfi.sub$value * NA
    dfi.sub[seq(2, nrow(dfi), by = 3), "value"] <- c(p10.cmip5.sub[1:9], WLp10.cmip5.sub[1:4])
    ...
    dfj <- data.frame("term" = x, "value" = unname(do.call("c", list(aj, bj, dj, a1j, a2j, a3j, a4j)))[ind])
    
    dfj.sub <- dfj 
    dfj.sub$value <- dfj.sub$value * NA
    dfj.sub[seq(2, nrow(dfj), by = 3), "value"] <- c(p90.cmip5.sub[1:9], WLp90.cmip5.sub[1:4])
    
    col <- col[ind]
    ylim <- c(floor(min(dfi$value)) - 1, ceiling(max(dfj$value)) + 1); step <- 1
    ylab <- bquote(Delta*"T(ºC)")
    
    bp <-xyplot(value~term, data = df, ylim = ylim, pch = 19, ylab = ylab, scales=list(x=list(rot=90)),
                col = col, cex = 1, xlab = "", #, 
                main = region,
                panel = function(...){
    ...
    WL.cmip5 <- lapply(scatter.seasons, function(s) suppressMessages(computeDeltas(project, var = "tas", experiment, season = s, ref.period, periods = WL, area, region)[[1]]))
    WL.cmip5.b <- lapply(scatter.seasons, function(s) suppressMessages(computeDeltas(project, var = "pr", experiment, season = s, ref.period, periods = WL, area, region)[[1]]))
    ...
    WL.cmip6 <- lapply(scatter.seasons, function(s) suppressMessages(computeDeltas(project, var = "tas", experiment, season = s, ref.period, periods = WL, area, region)[[1]]))
    WL.cmip6.b <- lapply(scatter.seasons, function(s) suppressMessages(computeDeltas(project, var = "pr", experiment, season = s, ref.period, periods = WL, area, region)[[1]]))
    ...
    WL.cordex <- lapply(scatter.seasons, function(s) suppressMessages(computeDeltas(project, var = "tas", experiment, season = s, ref.period, periods = WL, area, region)))
    WL.cordex.b <- lapply(scatter.seasons, function(s) suppressMessages(computeDeltas(project, var = "pr", experiment, season = s, ref.period, periods = WL, area, region)))
    ...
    sp <- lapply(1:length(scatter.seasons), function(k) {
      ...
      col <- c(rgb(0.55,0,0.55,0.5), 
               ...
               rgb(0, 0, 0), 
               rgb(0.5, 0.3, 0.16))
      ylim <- c(min(c(WLp10.cmip5.b[[k]],WLp10.cmip6.b[[k]]))-1, max(c(WLp90.cmip5.b[[k]], WLp90.cmip6.b[[k]]))+1)
      xyplot(x~y, data = dfs, ylim = ylim, pch = 19, 
             # scales = list(x = list(rot = 90)),
             col = col, cex = 1, xlab = bquote(Delta*"T(ºC)"), ylab = bquote(Delta*"P(%)"),
             main = list(seas.names[[k]], cex = 1),
             panel = function(...){
               # panel.abline(h = do.call("seq", as.list(c(ylim, step))),
               #              col = "gray65", lwd = 0.5, lty = 2)
               panel.segments(WLp10.cmip6[[k]], WLmediana.cmip6.b[[k]], WLp90.cmip6[[k]], WLmediana.cmip6.b[[k]], col = col[9:12], lwd = 5)
               panel.segments(WLmediana.cmip6[[k]], WLp10.cmip6.b[[k]], WLmediana.cmip6[[k]], WLp90.cmip6.b[[k]], col = col[9:12], lwd = 5)
               panel.segments(min(WLp10.cmip5[[k]])-1, 0, max(WLp90.cmip6[[k]])+1, 0, lty = 3)
               panel.segments(WLp10.cmip5[[k]], WLmediana.cmip5.b[[k]], WLp90.cmip5[[k]], WLmediana.cmip5.b[[k]], col = col[1:4], lwd = 5)
               panel.segments(WLmediana.cmip5[[k]], WLp10.cmip5.b[[k]], WLmediana.cmip5[[k]], WLp90.cmip5.b[[k]], col = col[1:4], lwd = 5)
               panel.segments(WLp10.cordex[[k]], WLmediana.cordex.b[[k]], WLp90.cordex[[k]], WLmediana.cordex.b[[k]], col = col[5:8], lwd = 2)
               panel.segments(WLmediana.cordex[[k]], WLp10.cordex.b[[k]], WLmediana.cordex[[k]], WLp90.cordex.b[[k]], col = col[5:8], lwd = 2)
               panel.xyplot(...)
             })
    })
    c(list(bp), sp)
